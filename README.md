...
> [!NOTE]
> `ko-sqlguard`의 코어 정책은 언어 중립적입니다. `ko-`는 한국어 SQL 문법을 뜻하지 않고,
> Prompt·PII·Output·Redteam과 함께 구성할 `KoGuard` 제품군의 네임스페이스입니다.
> 기본 정책에 주민등록번호 등 일부 한국형 민감 컬럼명이 포함되지만, 핵심 차별점은
> 한국어 구문 처리가 아니라 LLM SQL을 위한 통합 실행 전 정책입니다.
...
LLM의 일반 답변은 텍스트지만 SQL 도구 호출은 데이터베이스가 실제로 실행하는 명령이다.
모델이 문법상 올바른 `SELECT`를 만들었더라도 허용되지 않은 고객 컬럼을 조회하거나, 시스템
카탈로그를 정찰하거나, 거대한 조인으로 자원을 소모할 수 있다. 프롬프트에 “읽기만 하라”고
적는 것은 실행 권한 통제가 아니며, read-only DB role도 민감한 읽기와 고비용 조회까지
구분해 주지는 않는다.

`ko-sqlguard`는 모델과 DB 사이에서 SQL을 실행하기 전에 AST로 해석하고, 애플리케이션이 정한
정책 안에 있다는 것을 증명할 수 있을 때만 전달한다. SQL 오류를 고치는 생성기나 DB 권한을
대체하는 제품이 아니라, **LLM이 가진 넓은 SQL 생성 능력을 좁은 업무 권한으로 축소하는
정책 집행 지점**이다.

## 왜 별도 저장소인가

분리 이유는 한국어와 영어의 차이가 아니라 **검사 대상과 운영 책임이 다르기 때문**이다.
Prompt/Output 가드는 자연어 신호를 검사하지만 SQLGuard는 파싱된 AST, DB dialect, 스키마
allowlist와 실행계획을 다룬다. 따라서 의존성, 정책 버전, 회귀 corpus, 배포 권한과 장애
처리를 독립적으로 검증하고 릴리스해야 한다. 또한 전체 KoGuard를 도입하지 않는
Text-to-SQL·에이전트 서비스도 이 경계만 단독으로 사용할 수 있다.

| 구분 | 주된 역할 |
|---|---|
| SQLGlot | SQL 파싱, AST 분석과 변환을 제공하는 범용 기반 |
| 개별 SQL validator | 특정 문장·predicate 등 선택한 규칙을 조합해 검증 |
| `ko-sqlguard` | read-only, 테이블·컬럼, 위험 함수·카탈로그, 결과 행 수와 비용을 하나의 fail-closed 정책 결정으로 통합 |
...
| `guard.check()` (파싱 전) | `query_too_many_characters` / `query_too_many_bytes` | `max_query_chars`(기본 100,000)와 UTF-8 `max_query_bytes`(기본 400,000) 상한을 넘는 입력을 parser 작업 전에 CRITICAL BLOCK |
...
| `check_pii_columns` | `pii_column` | 오프라인 스키마에서 분류한 테이블별 PII 컬럼, 별칭·CTE 내부·`SELECT *`·whole-row·`USING`/`NATURAL JOIN` 접근 |
...
policy = GuardPolicy(
    allowed_tables={
        "customers": ["id", "성명", "주민등록번호", "product_name"],
    },
    pii_columns={
        "customers": ["성명", "주민등록번호"],
    },
...
guard.check("SELECT 주민등록번호 FROM customers").verdict  # BLOCK
guard.check("SELECT * FROM customers").verdict              # BLOCK
guard.check("SELECT product_name FROM customers").verdict   # TRANSFORM/PASS
```

`pii_columns`는 신뢰할 수 있는 schema snapshot에서 생성해야 합니다. 런타임 SQL 검사 중
카탈로그를 조회하거나 컬럼명 부분 문자열만으로 정책을 만들면 안 됩니다. 결과 행의 PII
lineage와 마스킹은 별도 결과 가드가 담당합니다.

`result.forward_safe`는 `result.sql`을 DB 경로로 전달해도 되는지를 명시한다. 원문 SQL과
상세 reason을 로그/metrics로 보내지 말고 `result.to_telemetry()`의 `verdict`,
`forward_safe`, `violation_codes`, `highest_severity`만 사용한다.

운영 allowlist 계약은 배포 시 명시적으로 검증한다. `allowed_tables={}`는 유효하며 모든
테이블 접근을 차단한다. 반면 기존 호환성을 위해 남아 있는 `allowed_tables=None`은 이 검증에서
거부된다.

```python
policy = GuardPolicy(allowed_tables={"orders": ["id", "status"]})
